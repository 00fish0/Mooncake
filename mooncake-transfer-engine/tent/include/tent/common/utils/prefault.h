...
inline PrefaultResult prefaultPages(void** pages, int n,
    ...
    if (allow_madvise) {
    {
        ...
        unsigned madvise_threads =
            static_cast<unsigned>((aligned_len + madvise_chunk - 1) /
                                  madvise_chunk);
        ...
        if (madvise_threads == 1) {
            ...
            for (unsigned t = 0; t < madvise_threads; ++t) {
                ...
                madv_threads.emplace_back([t, chunk_addr, chunk_len,
                                           &madv_rcs, &madv_errs]() {
                    int rc = madvise(chunk_addr, chunk_len,
                                     MADV_POPULATE_WRITE);
                    madv_rcs[t] = rc;
        ...
        auto madv_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(madv_end -
                                                                  madv_start)
                .count();
        if (madv_rc == 0) {
    ...
    if (allow_mlock) {
    {
        ...
        auto mlock_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(mlock_end -
                                                                  mlock_start)
                .count();
        if (mlock_rc == 0) {
    ...
    if (allow_touch) {
    {
        ...
        if (n < options.touch_single_thread_threshold_pages) num_threads = 1;
